...
// Money, not *Money), so it always serializes as an object per the
...
func TestVAPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrBadRequest) {
		t.Errorf("VAPayment() error = %v, want errors.Is(err, ErrBadRequest)", err)
	}
}
...
func TestVAPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrInternalServerError) {
		t.Errorf("VAPayment() error = %v, want errors.Is(err, ErrInternalServerError)", err)
	}
}
