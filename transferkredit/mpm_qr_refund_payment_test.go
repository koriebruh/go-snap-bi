...
func TestQRMPMRefundPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("QRMPMRefundPayment() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestQRMPMRefundPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("QRMPMRefundPayment() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
