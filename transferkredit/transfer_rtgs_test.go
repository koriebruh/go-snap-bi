...
func TestRTGSTransfer_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("RTGSTransfer() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestRTGSTransfer_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("RTGSTransfer() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
