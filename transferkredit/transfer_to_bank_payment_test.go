...
func TestTransferToBankPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("TransferToBankPayment() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestTransferToBankPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("TransferToBankPayment() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
