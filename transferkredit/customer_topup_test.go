...
func TestCustomerTopUp_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("CustomerTopUp() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestCustomerTopUp_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("CustomerTopUp() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
