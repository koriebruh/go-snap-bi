...
func TestCardRegistrationUnbinding_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("CardRegistrationUnbinding() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestCardRegistrationUnbinding_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("CardRegistrationUnbinding() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
