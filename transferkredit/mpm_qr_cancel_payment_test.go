...
func TestQRMPMCancelPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("QRMPMCancelPayment() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestQRMPMCancelPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("QRMPMCancelPayment() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
