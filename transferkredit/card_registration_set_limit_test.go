...
func TestCardRegistrationSetLimit_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("CardRegistrationSetLimit() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestCardRegistrationSetLimit_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("CardRegistrationSetLimit() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
