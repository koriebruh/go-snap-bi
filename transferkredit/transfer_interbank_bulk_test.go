   ...
   "additionalInfo":{"channel":"mobilephone"}
	...
	resp, err := InterbankBulkTransfer(context.Background(), tr, hb, InterbankBulkTransferRequest{
		...
		BulkObject: []InterbankBulkTransferItem{
			{PartnerReferenceNo: "pr-1", BankCode: "014", BeneficiaryAccountNo: "1234567890", BeneficiaryAccountName: "Jane Doe", Amount: snap.Money{Value: "50000.00", Currency: "IDR"}},
		},
	})
	if err != nil {
...
func TestInterbankBulkTransfer_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("InterbankBulkTransfer() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestInterbankBulkTransfer_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("InterbankBulkTransfer() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
