...
func TestAccountCreation_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("AccountCreation() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestAccountCreation_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("AccountCreation() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
