...
func TestAccountInquiryInternal_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("AccountInquiryInternal() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestAccountInquiryInternal_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("AccountInquiryInternal() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
