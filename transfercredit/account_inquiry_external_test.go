...
func TestAccountInquiryExternal_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("AccountInquiryExternal() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestAccountInquiryExternal_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("AccountInquiryExternal() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
