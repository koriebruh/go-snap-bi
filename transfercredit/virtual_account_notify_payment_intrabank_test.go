...
func TestVANotifyPaymentIntrabank_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("VANotifyPaymentIntrabank() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestVANotifyPaymentIntrabank_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("VANotifyPaymentIntrabank() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
