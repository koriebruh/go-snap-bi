...
// is a mandatory nested object (plain snap.Money, not *snap.Money), so it
...
func TestTransferToOTCTransferStatus_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("TransferToOTCTransferStatus() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestTransferToOTCTransferStatus_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("TransferToOTCTransferStatus() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
