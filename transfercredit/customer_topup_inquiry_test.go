...
func TestAccountInquiryCustomerTopUp_ParsesResponse(t *testing.T) {
   ...
   "feeType":"01"
	...
	want := AccountInquiryCustomerTopUpResponse{
		...
		MinAmount:              &snap.Money{Value: "10000.00", Currency: "IDR"},
		MaxAmount:              &snap.Money{Value: "1000000.00", Currency: "IDR"},
		Amount:                 &snap.Money{Value: "100000.00", Currency: "IDR"},
...
// present on the wire as an object (mandatory nested snap.Money is a plain
...
func TestAccountInquiryCustomerTopUp_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("AccountInquiryCustomerTopUp() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestAccountInquiryCustomerTopUp_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("AccountInquiryCustomerTopUp() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
