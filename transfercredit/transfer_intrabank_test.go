...
func TestIntrabankTransfer_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("IntrabankTransfer() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestIntrabankTransfer_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("IntrabankTransfer() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
