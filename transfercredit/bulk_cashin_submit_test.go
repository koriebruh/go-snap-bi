...
func TestSubmitBulkCashIn_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("SubmitBulkCashIn() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestSubmitBulkCashIn_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("SubmitBulkCashIn() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
