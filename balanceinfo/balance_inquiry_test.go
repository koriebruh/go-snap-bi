...
func TestBalanceInquiry_ParsesWorkedExampleResponse(t *testing.T) {
	...
	tr := &snap.Transport{}
	resp, err := BalanceInquiry(context.Background(), tr, snaptest.TestHeaderBuilder(server.URL), BalanceInquiryRequest{
		PartnerReferenceNo: "2020102900000000000001",
...
func TestBalanceInquiry_RequestBodyRoundTrips(t *testing.T) {
	...
	if _, err := BalanceInquiry(context.Background(), tr, snaptest.TestHeaderBuilder(server.URL), req); err != nil {
		t.Fatalf("BalanceInquiry() error = %v", err)
	}
...
func TestBalanceInquiry_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	tr := &snap.Transport{}
	_, err := BalanceInquiry(context.Background(), tr, snaptest.TestHeaderBuilder(server.URL), BalanceInquiryRequest{AccountNo: "123"})
	if err == nil {
		t.Fatal("BalanceInquiry() error = nil, want non-nil for a non-2xx responseCode")
	}
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("BalanceInquiry() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestBalanceInquiry_NonTwoXXWithNoResponseCodeIsError(t *testing.T) {
	...
	tr := &snap.Transport{}
	resp, err := BalanceInquiry(context.Background(), tr, snaptest.TestHeaderBuilder(server.URL), BalanceInquiryRequest{AccountNo: "123"})
	if err == nil {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("BalanceInquiry() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
...
// BalanceInquiry, snap.CheckResponseStatus's 2xx passthrough) had never once
...
// test for the exported snap.ErrUnmappedResponseCode: a gateway status SNAP's
...
func TestBalanceInquiry_UnmappedStatusIsErrorsIsMatchable(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrUnmappedResponseCode) {
		t.Errorf("BalanceInquiry() error = %v, want errors.Is(err, snap.ErrUnmappedResponseCode)", err)
	}
}
...
// — snap.Transport.Do returned before Envelope (and its StatusCode) was ever
...
func TestBalanceInquiry_NonTwoXXWithNonJSONBodyIsError(t *testing.T) {
	tests := []struct {
		...
		{"HTML WAF page", http.StatusUnauthorized, "<html><body>401 Unauthorized</body></html>", snap.ErrUnauthorized},
		{"empty body", http.StatusServiceUnavailable, "", snap.ErrServiceUnavailable},
	}
