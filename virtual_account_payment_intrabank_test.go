   ...
   "virtualAccountdata":{
	...
	want := VAPaymentIntrabankResponse{
		...
		VirtualAccountData: &VAPaymentIntrabankData{
			...
			PartnerReferenceNo:      "ref-1",
			PaidAmount:              &Money{Value: "100000.00", Currency: "IDR"},
			CumulativePaymentAmount: &Money{Value: "200000.00", Currency: "IDR"},
			PaidBills:               "3F",
			PaymentStatus:           "Settled",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := VAPaymentIntrabank(context.Background(), tr, hb, VAPaymentIntrabankRequest{
				...
				PaidAmount:  Money{Value: "1.00", Currency: "IDR"},
			})
			if err != nil {
				t.Fatalf("VAPaymentIntrabank() error = %v", err)
			}
...
// Money, not *Money), so it always serializes as an object.
func TestVAPaymentIntrabank_MandatoryFieldsAlwaysSerialized(t *testing.T) {
	...
	if _, ok := got["paidAmount"].(map[string]any); !ok {
		t.Errorf(`wire body["paidAmount"] = %v, want an object (mandatory nested Money is a plain struct)`, got["paidAmount"])
	}
...
func TestVAPaymentIntrabank_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrBadRequest) {
		t.Errorf("VAPaymentIntrabank() error = %v, want errors.Is(err, ErrBadRequest)", err)
	}
}
...
func TestVAPaymentIntrabank_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrInternalServerError) {
		t.Errorf("VAPaymentIntrabank() error = %v, want errors.Is(err, ErrInternalServerError)", err)
	}
}
