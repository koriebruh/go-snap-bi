...
// *Money Amount field is dropped from the wire when nil, unlike the
// Mandatory plain-Money Amount fields on other Trigger Transfer
// endpoints.
...
func TestRequestForPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrBadRequest) {
		t.Errorf("RequestForPayment() error = %v, want errors.Is(err, ErrBadRequest)", err)
	}
}
...
func TestRequestForPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrInternalServerError) {
		t.Errorf("RequestForPayment() error = %v, want errors.Is(err, ErrInternalServerError)", err)
	}
}
