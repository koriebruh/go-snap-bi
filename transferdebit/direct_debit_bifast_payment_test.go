...
func TestDirectDebitBIFASTPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("DirectDebitBIFASTPayment() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestDirectDebitBIFASTPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("DirectDebitBIFASTPayment() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
