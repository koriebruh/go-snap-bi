...
func TestCPMCancelPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("CPMCancelPayment() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestCPMCancelPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("CPMCancelPayment() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
