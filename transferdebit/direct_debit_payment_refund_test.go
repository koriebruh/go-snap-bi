...
func TestDirectDebitPaymentRefund_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("DirectDebitPaymentRefund() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestDirectDebitPaymentRefund_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("DirectDebitPaymentRefund() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
