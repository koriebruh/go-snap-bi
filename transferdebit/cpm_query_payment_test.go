...
func TestCPMQueryPayment_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("CPMQueryPayment() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestCPMQueryPayment_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("CPMQueryPayment() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
