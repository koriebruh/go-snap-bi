...
func TestAuthVoidQuery_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("AuthVoidQuery() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestAuthVoidQuery_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("AuthVoidQuery() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
