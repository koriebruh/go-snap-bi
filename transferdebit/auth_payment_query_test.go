...
// as always-serializing; Amount is Optional (*snap.Money, omitempty) and
...
func TestAuthPaymentQuery_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("AuthPaymentQuery() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestAuthPaymentQuery_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("AuthPaymentQuery() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
