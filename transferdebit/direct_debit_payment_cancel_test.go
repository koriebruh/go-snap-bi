...
func TestDirectDebitPaymentCancel_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("DirectDebitPaymentCancel() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestDirectDebitPaymentCancel_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("DirectDebitPaymentCancel() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
