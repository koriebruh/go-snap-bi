...
func TestCPMGenerateQR_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("CPMGenerateQR() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestCPMGenerateQR_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("CPMGenerateQR() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
