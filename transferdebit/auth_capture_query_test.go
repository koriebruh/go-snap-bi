...
func TestAuthCaptureQuery_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("AuthCaptureQuery() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestAuthCaptureQuery_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("AuthCaptureQuery() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
