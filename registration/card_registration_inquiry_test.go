...
func TestCardRegistrationInquiry_UsesGETWithNoBodyRegardlessOfCallerHeaderBuilder(t *testing.T) {
	...
	hb := snaptest.TestHeaderBuilder(server.URL) // Method: http.MethodPost, by default
	hb.EndpointURL = server.URL + "/v1.0/registration-card-inquiry"
	...
	if gotMethod != http.MethodGet {
		t.Errorf("request method = %q, want %q even though the caller's snap.HeaderBuilder had Method=POST", gotMethod, http.MethodGet)
	}
	if gotContentLength != 0 {
		t.Errorf("request Content-Length = %d, want 0 even though the caller's snap.HeaderBuilder had a Body set", gotContentLength)
	}
}
...
func TestCardRegistrationInquiry_RejectsEmptyEndpointURL(t *testing.T) {
	hb := snaptest.TestHeaderBuilder("http://unused.invalid")
	hb.EndpointURL = ""
	tr := &snap.Transport{}
	_, err := CardRegistrationInquiry(context.Background(), tr, hb, "cust-1")
	if !errors.Is(err, errInvalidEndpointURL) {
...
func TestCardRegistrationInquiry_SignatureCoversGETRequest(t *testing.T) {
	...
	stringToSign := snap.BuildStringToSignTransaction(http.MethodGet, wantEndpoint, hb.AccessToken, nil, gotTimestamp, hb.Symmetric)
	wantSignature := snap.SignSymmetric(hb.ClientSecret, stringToSign)
...
func TestCardRegistrationInquiry_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrBadRequest) {
		t.Errorf("CardRegistrationInquiry() error = %v, want errors.Is(err, snap.ErrBadRequest)", err)
	}
}
...
func TestCardRegistrationInquiry_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, snap.ErrInternalServerError) {
		t.Errorf("CardRegistrationInquiry() error = %v, want errors.Is(err, snap.ErrInternalServerError)", err)
	}
}
