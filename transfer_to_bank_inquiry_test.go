...
// mandatory nested object (plain Money, not *Money), so it always
...
func TestTransferToBankAccountInquiry_NonTwoXXResponseCodeIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrBadRequest) {
		t.Errorf("TransferToBankAccountInquiry() error = %v, want errors.Is(err, ErrBadRequest)", err)
	}
}
...
func TestTransferToBankAccountInquiry_NonTwoXXStatusWithTwoXXBodyIsError(t *testing.T) {
	...
	if !errors.Is(err, ErrInternalServerError) {
		t.Errorf("TransferToBankAccountInquiry() error = %v, want errors.Is(err, ErrInternalServerError)", err)
	}
}
