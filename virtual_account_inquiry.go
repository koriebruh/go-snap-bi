...
// carry every field HeaderBuilder needs except Body, which VAInquiry
// sets itself so the exact marshaled bytes are used for both signing
// and the wire body.
func VAInquiry(ctx context.Context, t *Transport, hb HeaderBuilder, req VAInquiryRequest) (VAInquiryResponse, error) {
